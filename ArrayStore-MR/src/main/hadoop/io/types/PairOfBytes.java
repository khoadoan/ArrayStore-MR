 ...
 * @author Jimmy Lin
 */
public class PairOfBytes implements WritableComparable<PairOfBytes> {
  private byte leftElement, rightElement;
   ...
   */
  public PairOfBytes(byte left, byte right) {
    set(left, right);
  }
  ...
  public void readFields(DataInput in) throws IOException {
    leftElement = in.readByte();
    rightElement = in.readByte();
  }
  ...
  public void write(DataOutput out) throws IOException {
    out.writeByte(leftElement);
    out.writeByte(rightElement);
  }
  ...
  public void setLeftElement(byte left) {
    leftElement = left;
  }
  
  public void setRightElement(byte right) {
    rightElement = right;
  }
   ...
   * @param right the right element
   */
  public void set(byte left, byte right) {
    leftElement = left;
    rightElement = right;
  }
  ...
  public boolean equals(Object obj) {
    PairOfBytes pair = (PairOfBytes) obj;
   ...
   *         <code>obj</code>.
   */
  public int compareTo(PairOfBytes pair) {
    byte pl = pair.getLeftElement();
    byte pr = pair.getRightElement();
   ...
   */
  public PairOfBytes clone() {
    return new PairOfBytes(this.leftElement, this.rightElement);
  }
  ...
  public static class Comparator extends WritableComparator {
    ...
    public int compare(byte[] b1, int s1, int l1, byte[] b2, int s2, int l2) {
      byte thisLeftValue = b1[s1];
      byte thatLeftValue = b2[s2];

      if (thisLeftValue == thatLeftValue) {
        byte thisRightValue = b1[s1 + 1];
        byte thatRightValue = b2[s2 + 1];
  ...
  static { // register this comparator
    WritableComparator.define(PairOfBytes.class, new Comparator());
  }
}
