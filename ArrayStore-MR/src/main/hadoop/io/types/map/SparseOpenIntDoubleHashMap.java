package hadoop.io.types.map;

import io.type.map.AbstractIntHashMap;
...
public class SparseOpenIntDoubleHashMap extends AbstractIntHashMap implements
    WritableComparable<SparseOpenIntDoubleHashMap> {
   ...
   */
  public SparseOpenIntDoubleHashMap() {
   ...
   */
  public SparseOpenIntDoubleHashMap(int initialCapacity) {
   ...
   *         .
   */
  public SparseOpenIntDoubleHashMap(int initialCapacity, double minLoadFactor, double maxLoadFactor) {
...
//    }
   ...
   * <p>
   * This method never need be called; it is for performance tuning only. Calling this method before
   * <tt>put()</tt>ing a large number of associations boosts performance, because the receiver will
   * grow only once instead of potentially many times and hash collisions get less probable.
   * 
   ...
   * guarantee</i> to use the <i>same</i> order defined by this method, even if it is no particular
   * order. This is necessary so that, for example, methods <tt>keys</tt> and <tt>values</tt> will
   * yield association pairs, not two uncorrelated lists.
   * 
   ...
   * guaranteed to be <i>identical</i> to the order used by method {@link #forEachKey(IntProcedure)}
   * .
   * 
   * @param procedure the procedure to be applied. Stops iteration if the procedure returns
   *        <tt>false</tt>, otherwise continues.
   ...
   * with {@link #containsKey(int)} whether the given key has a value associated or not, i.e.
   ...
   *         Returns -index-1 if the key is already contained at slot index. Therefore, if the
   *         returned index < 0, then it is already contained at slot -index-1. If the returned
   *         index >= 0, then it is NOT already contained and should be inserted at slot index.
   */
   ...
   * check with {@link #containsValue(double)} whether there exists an association from a key to
   * this value. Search order is guaranteed to be <i>identical</i> to the order used by method
  ...
  @Override
  public int compareTo(SparseOpenIntDoubleHashMap other) {
    return 0;
  }
}
